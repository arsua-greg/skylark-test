...
export default function ReservationForm() {
  ...
  const submitHandler = (e: any) => {
    ...
    if (name.trim() === '' || phoneNumber.trim() === '' || email.trim() === '' || value.trim() === '') {
      setValidateError({
        name: name.trim() === '',
  ...
  return (
    <Fragment>
      ...
      >
        <ReservationDetails />
        <div  className={`border py-3 mt-7 md:mt-10 bg-[#F71B1B1A] ${validateError.name || validateError.phoneNumber || validateError.email || validateError.value ? 'block' : "hidden" }`}>
        ...
        <p ref={errorRef} className="font-bold text-lg mt-6">ご来店者情報</p>
        ...
        <div className="mt-6">
          ...
          ></textarea>
          <p className={`text-[13px] text-[#F71B1B] leading-tight pt-1 ${validateError.value} ${validateError.value ? 'block' : 'hidden'} `}>
