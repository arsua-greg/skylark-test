...
const Steps: React.FC<StepProps> = ({ active }) => {
  return (
    <div className="flex items-center text-center md:max-w-[1120px] mx-auto overflow-hidden">
      <div
        className={`w-1/3 bg-[#D9D9D9] py-3  relative ${
          active === 1 ? "isActive" : ""
        } stepArrow`}
      >
        {active === 1 ? <div className="stepArrow01"></div> : ""}
        <p className="md:text-lg text-[13px] leading-[22px] relative z-20">
          入力・確認
        </p>
      </div>
      <div
